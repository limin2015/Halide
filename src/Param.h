...
public:
    ...
    template <typename OTHER_TYPE, typename T2 = T, typename std::enable_if<!std::is_pointer<T2>::value>::type * = nullptr>
    Param(Type t, OTHER_TYPE val) :
        param(t, false, 0, Internal::make_entity_name(this, "Halide::Param<?", 'p')) {
        static_assert(!has_static_type, "Cannot use this ctor with an explicit type.");
        set<OTHER_TYPE>(val);
    }
    ...
    Param(Type t, const std::string &n, OTHER_TYPE val) :
        param(t, false, 0, n, /*is_explicit_name*/ true) {
        check_name();
        static_assert(!has_static_type, "Cannot use this ctor with an explicit type.");
        set<OTHER_TYPE>(val);
    ...
    * and a given min and max. */
    ...
    template <typename OTHER_TYPE>
    Param(Type t, OTHER_TYPE val, Expr min, Expr max) :
        param(t, false, 0, Internal::make_entity_name(this, "Halide::Param<?", 'p')) {
        static_assert(!has_static_type, "Cannot use this ctor with an explicit type.");
        set_range(min, max);
        set<OTHER_TYPE>(val);
    ...
    template <typename OTHER_TYPE>
    Param(Type t, const std::string &n, OTHER_TYPE val, Expr min, Expr max) :
        param(type_of<T>(), false, 0, n, /*is_explicit_name*/ true) {
        static_assert(!has_static_type, "Cannot use this ctor with an explicit type.");
        check_name();
        set_range(min, max);
